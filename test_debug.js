    ...
    boot();
        ...
        for (let attempt = 0; attempt < 1000; attempt += 1) {
            ...
            if (colPool.length >= 3) {
                validRowsFound++;
                const hasCountry = colPool.some(c => parseConstraint(c).type === 'country');
                const hasPosition = colPool.some(c => parseConstraint(c).type === 'position');
                if(validRowsFound <= 5) {
                    console.log("Rows:", rows, "ColPool:", colPool, "HasCountry:", hasCountry, "HasPos:", hasPosition);
                }
            }
        }
        console.log("Out of 1000 row picks, valid rows found:", validRowsFound);
