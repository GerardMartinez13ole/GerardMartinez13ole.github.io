    ...
    function generateBoard(seed, blockedCountryCodes, config) {
      ...
      for (let attempt = 0; attempt < maxTries; attempt += 1) {
        ...
        const colPool = state.constraints.filter((constraint) => !rows.includes(constraint));
    ...
    boot();
        ...
        const teams = state.teamConstraints;
        const countries = state.constraints.filter(c => parseConstraint(c).type === 'country');
        
        console.log("Teams:", teams.length, "Countries:", countries.length);
        
        let foundCombos = 0;
        
        // Let's just do a random search but check if any exists.
        // We'll iterate combinations of 3 teams and then find compatible countries.
        // For each team, we find which countries it has players for.
        // A country is "compatible" with a team if optionsForPair(team, country) > 0.
        // To find 3 countries, we intersect the compatible countries of 3 teams.
            ...
            for (let j = i + 1; j < teams.length; j++) {
                for (let k = j + 1; k < teams.length; k++) {
                    ...
                    for (const c of countries) {
                        if (optionsForPair(t1, c).length > 0 &&
                            optionsForPair(t2, c).length > 0 &&
                            optionsForPair(t3, c).length > 0) {
                            sharedCountries.push(c);
                        }
                    }
                    
                    if (sharedCountries.length >= 3) {
                        foundCombos++;
                        if(foundCombos <= 5) {
                            console.log("Found teams:", t1, t2, t3, "with at least 3 shared countries:", sharedCountries);
                        }
                    }
                }
            }
        ...
        console.log("Total combinations of 3 teams that share at least 3 countries:", foundCombos);
