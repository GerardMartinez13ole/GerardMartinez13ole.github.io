...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.switchTab = (target, title) => {
    if (target !== 'nuevo' && window.editingCapturaId) {
      ...
      const especieAltre = document.getElementById('cap-especie-altre');
      if (especieAltre) {
        especieAltre.style.display = 'none';
        especieAltre.required = false;
      }
    }
  ...
  const especieSelect = document.getElementById('cap-especie');
  const especieAltre = document.getElementById('cap-especie-altre');
  if (especieSelect && especieAltre) {
    especieSelect.addEventListener('change', () => {
      if (especieSelect.value === 'Altre') {
        especieAltre.style.display = 'block';
        especieAltre.required = true;
      } else {
        especieAltre.style.display = 'none';
        especieAltre.required = false;
        especieAltre.value = '';
...
window.iniciarEdicionCaptura = async (id) => {
  ...
  // Tratar de seleccionar la especie en el desplegable
  let especieExiste = false;
  for (let i = 0; i < especieSelect.options.length; i++) {
    if (especieSelect.options[i].value === (data.especie || '')) {
      especieExiste = true;
      break;
    }
  }
  ...
  if (especieExiste) {
    ...
    especieSelect.value = 'Altre';
    especieAltre.style.display = 'block';
    especieAltre.value = data.especie;
    especieAltre.required = true;
  } else {
    especieSelect.value = '';
    especieAltre.style.display = 'none';
