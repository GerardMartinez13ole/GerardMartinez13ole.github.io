    ...
    boot();
        ...
        // Define a modified generateBoard to test
        function generateBoardTest(seed, blockedCountryCodes, config) {
          const random = mulberry32(seed);
          const maxTries = 2200;
          let best = null;
          const blockedCountries = new Set();
          ...
          for (let attempt = 0; attempt < maxTries; attempt += 1) {
            ...
            // OPTIMIZATION: Filter colPool to only those that intersect with all 3 rows
            ...
            if (!valid || minOptions < 1) continue;
            if (!canSolveWithoutRepeats(rows, cols)) continue;
        ...
        for(let attempt=0; attempt < 10; attempt++) {
            ...
            try {
               const board = generateBoardTest(seed, [], { teams: 0, countries: 3, positions: 0 });
               console.log("FOUND BOARD in seed attempt", attempt);
               console.log(board);
               found = true;
               break;
            } catch(e) {
        ...
        if(!found) console.log("Failed to find board after 10 seeds.");
